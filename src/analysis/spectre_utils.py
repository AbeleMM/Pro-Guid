...
import datasets.graph_generators as graph_generators
...
def betweennness_centrality_worker(param):
    G, bins = param
    clustering_coeffs_list = list(nx.betweenness_centrality(G).values())
    hist, _ = np.histogram(
        clustering_coeffs_list, bins=bins, range=(0.0, 1.0), density=False
    )
    return hist
...
def eigenvector_centrality_worker(param):
    G, bins = param
    clustering_coeffs_list = list(nx.eigenvector_centrality(G, max_iter=20000).values())
    hist, _ = np.histogram(
        clustering_coeffs_list, bins=bins, range=(0.0, 1.0), density=False
    )
    return hist
...
def laplacian_centrality_worker(param):
    G, bins = param
    clustering_coeffs_list = list(nx.laplacian_centrality(G).values())
    hist, _ = np.histogram(
        clustering_coeffs_list, bins=bins, range=(0.0, 1.0), density=False
    )
    return hist
...
def centrality_stats(
        ...
        type: Literal["betweenness", "eigenvector", "laplacian"],
        bins=100,
    ...
    if type == "betweenness":
        worker = betweennness_centrality_worker
    elif type == "eigenvector":
        worker = eigenvector_centrality_worker
    elif type == "laplacian":
        worker = laplacian_centrality_worker

    prev = datetime.now()
    if is_parallel:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for clustering_hist in executor.map(
                worker, [(G, bins) for G in graph_ref_list]
            ):
                sample_ref.append(clustering_hist)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for clustering_hist in executor.map(
                worker, [(G, bins) for G in graph_pred_list_remove_empty]
            ):
                sample_pred.append(clustering_hist)
        ...
        for i in range(len(graph_ref_list)):
            sample_ref.append(worker((graph_ref_list[i], bins)))

        for i in range(len(graph_pred_list_remove_empty)):
            sample_pred.append(worker((graph_pred_list_remove_empty[i], bins)))
...
def eval_acc_grid_graph(G_list):
    count = 0
    for gg in G_list:
        if is_grid_graph(gg):
            count += 1
...
def eval_acc_chordal_graph(G_list):
    return sum(nx.is_chordal(gg) for gg in G_list) / len(G_list)


def is_planar_graph(G):
    return nx.is_connected(G) and nx.is_planar(G)
    ...
    for fake_g in fake_graphs:
        ...
        for fake_old in fake_evaluated:
            try:
                ...
                signal.alarm(60)
                if nx.faster_could_be_isomorphic(fake_g, train_g) and \
                    nx.is_isomorphic(fake_g, fake_old):
                    count_non_unique += 1
                    unique = False
                    break
            except TimeoutError:
...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, datamodule, compute_emd, metrics_list):
        ...
        self.dataset_name = datamodule.cfg.dataset.name
    ...
    def forward(
        ...
        if n_nodes is not None:
            ...
            n_graphs = len(reference_graphs)
            seed = 0

            if dataset_name == "planar":
                reference_graphs = graph_generators.generate_planar_graphs(
                    n_graphs, n_nodes, seed=seed)
            elif dataset_name == "tree":
                reference_graphs = graph_generators.generate_tree_graphs(
                    n_graphs, n_nodes, seed=seed)
            elif dataset_name == "sbm":
                reference_graphs = graph_generators.generate_sbm_graphs(
                    n_graphs, n_nodes, seed=seed)
            elif dataset_name == "lobster":
                reference_graphs = graph_generators.generate_lobster_graphs(
                     n_graphs, n_nodes, seed=seed)
            elif dataset_name == "erdosrenyi":
                reference_graphs = graph_generators.generate_erdos_renyi_graphs(
                    n_graphs, n_nodes, seed=seed)
            else:
                raise AssertionError("Unsupported extrapolation dataset.")

        if local_rank == 0:
            print(
                f"Computing sampling metrics between {len(generated_graphs)} generated graphs "
                f"and {len(reference_graphs)} test graphs -- emd computation: {self.compute_emd}"
            )
        ...
        if "betw_centr" in self.metrics_list:
            if local_rank == 0:
                print("Computing betw_centr stats...")
            betw_centr = centrality_stats(
                reference_graphs,
                networkx_graphs,
                "betweenness",
                bins=100,
            ...
            to_log["betw_centr"] = betw_centr
            if wandb.run:
                wandb.run.summary["betw_centr"] = betw_centr

        if "eigv_centr" in self.metrics_list:
            if local_rank == 0:
                print("Computing eigv_centr stats...")
            eigv_centr = centrality_stats(
                reference_graphs,
                networkx_graphs,
                "eigenvector",
                bins=100,
            ...
            to_log["eigv_centr"] = eigv_centr
            if wandb.run:
                wandb.run.summary["eigv_centr"] = eigv_centr

        if "lapl_centr" in self.metrics_list:
            if local_rank == 0:
                print("Computing lapl_centr stats...")
            lapl_centr = centrality_stats(
                reference_graphs,
                networkx_graphs,
                "laplacian",
                bins=100,
            ...
            to_log["lapl_centr"] = lapl_centr
            if wandb.run:
                wandb.run.summary["lapl_centr"] = lapl_centr
        ...
        if "chordal" in self.metrics_list:
            if local_rank == 0:
                print("Computing chordal accuracy...")
            chordal_acc = eval_acc_chordal_graph(networkx_graphs)
            to_log["chordal_acc"] = chordal_acc
            if wandb.run:
                wandb.run.summary["chordal_acc"] = chordal_acc

        if (
            "sbm" in self.metrics_list
            or "planar" in self.metrics_list
            or "tree" in self.metrics_list
            or "lobster" in self.metrics_list
            or "chordal" in self.metrics_list
        ):
            if local_rank == 0:
            ...
            elif "chordal" in self.metrics_list:
                validity_func = nx.is_chordal
            else:
                validity_func = None
            (
                frac_unique,
        ...
        ratios = compute_ratios(
            ...
            metrics_keys=[
                m for m in (
                    ...
                    "betw_centr", "eigv_centr", "lapl_centr")
                if m in to_log
            ],
        )
        ...
        if local_rank == 0:
            print("Sampling statistics", {k: round(v, 4) for k, v in to_log.items()})
        if wandb.run:
...
class ErdosRenyiSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule):
        super().__init__(
            datamodule=datamodule,
            compute_emd=False,
...
class ChordalSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule):
        super().__init__(
            datamodule=datamodule,
            compute_emd=False,
            metrics_list=[
                "degree", "clustering", "orbit", "spectre", "wavelet", "betw_centr", "chordal"],
        )
