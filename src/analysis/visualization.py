...
class MolecularVisualization:
    ...
    def visualize_chain_edge_prob(self, path, edge_probs, times, ref_edges=None):
        return _render_edge_prob_gif(path, edge_probs, times, ref_edges)
...
class NonMolecularVisualization:
    ...
    def visualize_chain_edge_prob(self, path, edge_probs, times, ref_edges=None):
        _render_edge_prob_gif(path, edge_probs, times, ref_edges)


def _render_edge_prob_gif(path, edge_probs, times, ref_edges):
    if ref_edges is None:
        matrices = edge_probs
        cmap = "viridis"
        title_prefix = "Prediction"
    else:
        matrices = np.abs(edge_probs - ref_edges)
        cmap = "Reds"
        title_prefix = "Error"
    ...
    for idx, matrix in enumerate(matrices):
        file_name = os.path.join(path, f"fram_edge_prob_{idx}.png")
        fig, ax = plt.subplots()
        ax.axis("off")
        ax.set_title(f"{title_prefix} @ t = {times[idx]:.2f}")
        im = ax.matshow(matrix, vmin=0.0, vmax=1.0, cmap=cmap)
    ...
    imgs = [imageio.imread(fn) for fn in save_paths]
    gif_path = os.path.join(
        os.path.dirname(path), "{}_edge_prob.gif".format(path.split("/")[-1])
    )
    imgs.extend([imgs[-1]] * 10)
    imageio.mimsave(gif_path, imgs, subrectangles=True, duration=200)

    if wandb.run:
        wandb.log({"chain_edge_prob": wandb.Video(gif_path, fps=5, format="gif")})
