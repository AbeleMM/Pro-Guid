...
class SpectreGraphDataset(InMemoryDataset):
    ...
    def download(self):
        if self.dataset_name in ("sbm", "chordal"):
            ...
            raw_url = pathlib.Path(__file__).parents[2] / "raw_data" / "lobster.pkl"
        elif self.dataset_name == "erdosrenyi":
            raw_url = pathlib.Path(__file__).parents[2] / "raw_data" / "erdosrenyi.pkl"
        elif self.dataset_name == "planar_extrapolation":
            raw_url = "https://raw.githubusercontent.com/AndreasBergmeister/graph-generation/main/data/planar_extrapolation.pkl"
        else:
        ...
        if self.dataset_name in [
            "tree", "sbm", "planar", "chordal", "planar_extrapolation", "lobster", "erdosrenyi"]:
            ...
            if self.dataset_name == "chordal":
                train_data = [complete_to_chordal_graph(g)[0] for g in train_data]
                val_data = [complete_to_chordal_graph(g)[0] for g in val_data]
                test_data = [complete_to_chordal_graph(g)[0] for g in test_data]
