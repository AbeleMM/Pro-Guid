...
def main(cfg: DictConfig):
    ...
    if dataset_config["name"] in [
        ...
        from analysis.spectre_utils import (
            ...
            ErdosRenyiSamplingMetrics,
            ChordalSamplingMetrics
        )
        ...
        elif dataset_config["name"] in ("planar", "planar_extrapolation"):
            sampling_metrics = PlanarSamplingMetrics(datamodule)
        elif dataset_config["name"] == "tree":
            sampling_metrics = TreeSamplingMetrics(datamodule)
        elif dataset_config["name"] == "lobster":
            sampling_metrics = LobsterSamplingMetrics(datamodule)
        elif dataset_config["name"] == "erdosrenyi":
            sampling_metrics = ErdosRenyiSamplingMetrics(datamodule)
        elif dataset_config["name"] == "chordal":
            sampling_metrics = ChordalSamplingMetrics(datamodule)
        else:
    ...
    trainer = Trainer(
        ...
        logger=[],
        inference_mode=False  # use torch.no_grad instead of torch.inference_mode
    )
    ...
    if not cfg.general.test_only and cfg.general.generated_path is None:
        ...
        # Causes pytorch_lightning UserWarning:
        # attribute 'dataset_infos' removed from hparams because it cannot be pickled
        model = cast(GraphDiscreteFlowModel, torch.compile(model))
